...
def load_vin(datadir, mode, data, num_train, num_test, scale=50, train=True, load_instructions=False):
    ...
    terminal = np.repeat(terminal, scale, axis=0)
...
def vin_format_terminal(layouts, objects, terminal, s1, s2, label, scale=1):
    ...
    assert len(input_map) == len(s1)
    for i in range(len(input_map)):
        t = (torch.Tensor(input_map[i]), s1[i], s2[i], label[i])
        data.append(t)
    return data

def vin_format_instructions(layouts, objects, instructions, s1, s2, label):
    data = []
    input_map = np.concatenate((layouts, objects), axis=1)
    for i in range(len(input_map)):
        t = (torch.Tensor(input_map[i]), instructions[i], s1[i], s2[i], label[i])
        data.append(t)
    return data
...
def get_vin_data(values, goals, scaling_factor=1):
    ...
    startpos_s2 = []
    optimal_actions = []
    grid_size = values[0].shape[0]
    for val, g in zip(values, goals):
...
def get_optimal_action(s1, s2, values, grid_size=10):
    """
    x is the inner dimension of the 2D array
    """
    cell_values = []
    overflow = False
    ...
    # print cell_values
    cell_values = reorder(cell_values)
    # print cell_values
...
def overflow_check(i, j, grid_size=10):
    if i < 0 or j < 0:
        return True
    elif i > grid_size-1 or j > grid_size-1:
        return True
    else:
        return False

def load(mode, data, num_train, num_test, datadir):
    turk_path = os.path.join(datadir, mode)
    states, _ = initialize_states(turk_path)

    if data == 'human':
        annotation_path = os.path.join(datadir, mode + '.p')
